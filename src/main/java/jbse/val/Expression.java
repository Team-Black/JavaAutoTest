...
public final class Expression extends PrimitiveSymbolicComputed {
    ...
    public boolean equals(Object obj) {
        ...
        if (this.secondOp == null) {
            return other.secondOp == null;
        } else return this.secondOp.equals(other.secondOp);
    }

    public String toStringWithValuesNames(HashMap<String, String> map) {
        String res = "";
        res = getString(map, res, firstOp);
        if (!firstOp.toString().isEmpty()) {
            res += operator.toString();
        }
        res = getString(map, res, secondOp);
        return res;
    }

    private String getString(HashMap<String, String> map, String res, Primitive firstOp) {
        if (firstOp instanceof Expression) {
            res += "(" + ((Expression) firstOp).toStringWithValuesNames(map) + ")";
        } else {
            String opName = map.get(firstOp.toString());
            if (opName != null) {
                res += opName;
            } else {
                res += firstOp.toString();
            }
        }
        return res;
    }
